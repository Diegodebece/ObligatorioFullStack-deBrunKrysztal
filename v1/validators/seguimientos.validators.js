...
export const crearSeguimientoSchema = Joi.object({
    ...
    ratingPersonal: Joi.number().integer().min(1).max(10).allow(null).default(null).messages({
    ...
    serie: Joi.string().hex().length(24).required().messages({
        "string.base": "La serie debe ser un texto",
        "string.hex": "El id de la serie debe ser hexadecimal",
        "string.length": "El id de la serie debe tener 24 caracteres",
        "any.required": "La serie es obligatoria"
    }),
    episodioActual: Joi.number().integer().min(0).max(9999).allow(null).default(null).messages({
        "number.base": "El episodio actual debe ser un número",
        "number.integer": "El episodio actual debe ser un número entero"
    }),
    temporadaActual: Joi.number().integer().min(0).max(9999).allow(null).default(null).messages({
        "number.base": "La temporada actual debe ser un número",
        "number.integer": "La temporada actual debe ser un número entero"
    }),
    fechaInicio: Joi.date().allow(null).default(null).messages({
        "date.base": "La fecha de inicio debe ser una fecha válida"
    }),
    fechaFin: Joi.date().greater(Joi.ref("fechaInicio")).allow(null).default(null).messages({
